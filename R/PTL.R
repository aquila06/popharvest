...
#' @param alpha.fixed number ; number ; fixed estimate of age at first breeding (in years). To be used when \code{alpha.unif = FALSE} and \code{alpha.lognorm = FALSE}.
...
#' @param sd.alpha number ; standard deviation of age at first breeding. To be used only if \code{alpha.lognorm = TRUE}.
...
PTL <- function(full.option = FALSE,
  ...
  ## III. Calculations ----
  ## Step 1 : Create the end storage vectors to save results and load functions----
  Output1 <- NULL
  Output2 <- NULL
  ...
  # This step allows to repeat the series of calculations whithin the loop as many times as there are studied species setting temporary input values
  for (a in 1: NSp) { # Loop for calculations from 1 to the number of species studied
    ...
    living.rateT <- ifelse(!is.null(living.rate), living.rate[a], NA) # If the argument 'living.rate' is not null, then temporary setting the value of 'living.rate' of the index a in the object named 'living.rateT', else assign 'NA' to the object 'living.rateT'. The same philosophy is applied below.
    ...
    # This step allows to repeat the series of calculations whithin the loop as many times as there are objective factors
    for (b in 1: length(Fobj)){ # Loop for calculations according to the objective factor
      FobjT <- Fobj[b]
      ...
      if(is.null(Rmax.fixed) && is.null(mean.Rmax) && is.null(lambdaMax.fixed) && is.null(mean.lambdaMax)){ # If the arguments 'Rmax.fixed, 'mean.Rmax, 'lambdaMax', and "mean.lambdaMax" are null then :
        ...
        OutputT1 <- rmax(Nsim = Nsim,
                         ...
                         mass.lognorm = mass.lognorm, #modified the 2022-07-22 (was set to FALSE)
                         mean.mass = mean.massT,
                         sd.mass = sd.massT, #modified the 2022-07-22 (mean.massT changed to sd.massT)
                         type.p = type.pT,
                         type.e = type.eT,
                         surv.j = surv.jT,
      ...
      if(isTRUE(pop.lognorm)){ # If popsize is extracted from a normal distribution then :
        ...
        popsize.calc <- rlnorm(Nsim, pop.param[1], pop.param[2]) # 'Nsim' values are extracted from a normal distribution with with the mean 'mean.popT' and the standard deviation 'sd.popT'
        popsize.calc <- round(popsize.calc) # Round the values of 'popsize' object to the whole
      }
      ...
      Sim <- 1:Nsim # This object allows to known the value of the index a for each row of the data frame below
      OutputT2 <- data.frame(SP = a, Sim = Sim, Fobj = FobjT, popsize = popsize.calc, harvest = harvest.calc) # Creation of the OutputT2


      # Step 8 : Generation of estimated variable of 'theta' ----
      # New code added 20/07/2022
      ...
      var.storage <- c("theta.fixedT", "estim.thetaT", "e.density", "theta", "Rmax.calc", "mean.RmaxT", "sd.RmaxT", "lambdaMax.fixedT", "mean.lambdaMaxT",
                       "sd.lambdaMaxT", "min.popT", "max.popT", "mean.popT", "sd.popT",
                       "min.harvestT", "max.harvestT", "mean.harvestT", "sd.harvestT") # Save in an object all sumpplementary possible columns
      ...
      ## Step 10 : Reorder columns of the 'OutputT2' ----
      var.rmax<- c("theta.fixedT", "estim.thetaT", "e.density", "theta", "Rmax.calc", "mean.RmaxT", "sd.RmaxT", "lambdaMax.fixedT", "mean.lambdaMaxT", "sd.lambdaMaxT") # Order of the columns wanted in the final output
      ...
      reorder <- c("SP", "Sim", "Fobj", var.rmax.name, "popsize", var.pop.name, "harvest", var.harv.name) # Final order of the columns wanted in the final output
      OutputT2 <- data.frame(OutputT2[, reorder]) # Reorder the columns in the data frame named 'OutputT2'


      # Step 11 : Save all the datas from the 'OutputT2' in the data frame named 'Output2' ----
      Output2 <- rbind(Output2, OutputT2) # Combine by rows all temporary data frame 'OutputT2' for each index a of the loop and save in the data frame 'Output2'




    } # End of loop b (according to the objective factor)
  ...
  colnames(full.tab)[colnames(full.tab)=="Nsim"] <- "Sim" # Rename column
  colnames(full.tab)[colnames(full.tab)=="Rmax.calc"] <- "Rmax" # Rename column
